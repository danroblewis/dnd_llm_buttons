...
model = ChatOpenAI(base_url="http://cluster:32605/v1", model="llama3.2")
...
async def generate(request: GenerateRequest):
    if request.prompt:
        ...
        res = model.invoke(dm_messages + [HumanMessage(content="Continue with the story, taking into account the player's choice. Limit yourself to less than 50 words.")])
        dm_messages.append(AIMessage(content=res.content))
        player_messages.append(AIMessage(content=res.content))

    # Generate 3 new choices
    choices = [model.invoke(player_messages + [HumanMessage(content="Write a short description of what you will do next. Be creative. Less than 10 words.")]).content for i in range(3)]
